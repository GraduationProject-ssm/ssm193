...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=187)


# 第1章 绪论
## 1.1 课题背景
互联网发展至今，无论是其理论还是技术都已经成熟，而且它广泛参与在社会中的方方面面。它让信息都可以通过网络传播，搭配信息管理工具可以很好地为人们提供服务。所以各行业，尤其是规模较大的企业和学校等都开始借助互联网和软件工具管理信息，传播信息，共享信息等等，以此可以增强自身实力，提高在同行业当中的竞争能力，并从各种激烈的竞争中获取发展的机会。针对职工健康信息管理混乱，出错率高，信息安全性差，劳动强度大，费时费力等问题，经过分析和考虑，在目前的情况下，可以引进一款职工健康每日申报系统这样的现代化管理工具，这个工具就是解决上述问题的最好的解决方案。它不仅可以实时完成信息处理，还缩短职工健康信息管理流程，使其系统化和规范化。同时还可以减少工作量，节约职工健康信息管理需要的人力和资金。所以职工健康每日申报系统是信息管理环节中不可缺少的工具，它对管理者来说非常重要。
## 1.2 课题意义 
现如今，信息种类变得越来越多，信息的容量也变得越来越大，这就是信息时代的标志。近些年，计算机科学发展得也越来越快，而且软件开发技术也越来越成熟，因此，在生活中的各个领域，只要存在信息管理，几乎都有计算机的影子，可以说很多行业都采用计算机的方式管理信息。信息计算机化处理相比手工操作，有着保密性强，效率高，存储空间大，成本低等诸多优点。针对职工健康信息管理，采用职工健康每日申报系统可以有效管理，使信息管理能够更加科学和规范。

总之，在实际中使用职工健康每日申报系统，其意义如下：

第一点：职工健康每日申报系统的实际运用，可以帮助管理人员在短时间内完成信息处理工作；
...
第四点：可以降低信息管理成本；
## 1.3 研究内容
对职工健康每日申报系统设计制作，不仅需要技术支撑，也需要大量的理论研究。本文在对职工健康每日申报系统进行介绍时，将按照如下内容进行。

第一部分：介绍职工健康每日申报系统研究的背景意义，便于用户了解系统；

第二部分：介绍开发职工健康每日申报系统需要搭建的环境，包括技术和工具；

第三部分：介绍用户对职工健康每日申报系统的功能要求，以及对职工健康每日申报系统的性能要求等；
...
# 第2章 开发环境与技术
职工健康每日申报系统的编码实现需要搭建一定的环境和使用相应的技术，接下来的内容就是对职工健康每日申报系统用到的技术和工具进行介绍。
## 2.1 MYSQL数据库
...
### 3.1.2 经济可行性
本系统开发期间需要配置的软件环境，可以免费通过开发类官网下载安装，需要配置的硬件设备也不需要具备很高的性能，通常网吧电脑，或学校计算机机房的电脑都符合要求。因此，从经济方面考虑，职工健康每日申报系统开发可行。
### 3.1.3 操作可行性
职工健康每日申报系统根据用户使用习惯进行开发，设计的界面具有统一性，并具备优秀的导航功能。所以，只要会简单操作电脑的人员，可以无压力操作职工健康每日申报系统。
...
职工健康每日申报系统经过分析，确定了其需要设置管理员的角色，其操作的功能通过用例图展示（见下图）。管理员管理员工的健康登记信息，管理员工，省份以及市区信息。

![](/md/blog.005.png)

图3.5 管理员用例图

职工健康每日申报系统经过分析，确定了其需要设置员工的角色，其操作的功能通过用例图展示（见下图）。员工登记健康信息，管理健康登记信息，包括修改，查询。
...
## 4.2 功能结构设计
在前面分析的管理员功能的基础上，进行接下来的设计工作，最终展示设计的管理员结构图（见下图）。管理员管理员工的健康登记信息，管理员工，省份以及市区信息。

![](/md/blog.007.png)

图4.1 管理员功能结构图

在前面分析的员工功能的基础上，进行接下来的设计工作，最终展示设计的员工结构图（见下图）。员工登记健康信息，管理健康登记信息，包括修改，查询。
...
|shengfen|varchar(200)|否||省份|
|shiqu|varchar(200)|否||市区|
|xiangxidizhi|varchar(200)|否||详细地址|
|tiwen|float|否||体温|
|shentizhuangkuang|varchar(200)|是|NULL|身体状况|
|qitashuoming|longtext|是|NULL|其他说明|
表4.2 省份信息表
...
|id (主键)|bigint(20)|否||主键|
|addtime|timestamp|否|CURRENT\_TIMESTAMP|创建时间|
|shengfen|varchar(200)|是|NULL|省份|
表4.3 市区信息表
...
|shenfenzheng|varchar(200)|否||身份证|
...
## 5.1 管理员功能实现
### 5.1.1 员工管理
管理员进入指定功能操作区之后可以管理员工。其页面见下图。管理员在当前页面新增员工信息，修改员工的身份证，手机，邮箱，照片等资料，删除指定的员工资料。

![](/md/blog.015.png)

图5.1 员工管理页面
### 5.1.2 健康登记管理
管理员进入指定功能操作区之后可以管理员工的健康登记信息。其页面见下图。管理员可以查询员工的健康登记信息，可以修改员工的健康登记信息，包括省份，市区，详细地址以及员工的体温等信息，可以删除指定的员工健康登记信息。

![](/md/blog.016.png)

图5.2 健康登记管理页面
### 5.1.3 省份管理
管理员进入指定功能操作区之后可以管理省份信息。其页面见下图。管理员在本页面可以新增省份，修改与删除省份信息。

![](/md/blog.017.png)

图5.3 省份管理页面
### 5.1.4 市区管理
管理员进入指定功能操作区之后可以管理市区信息。其页面见下图。管理员新增市区信息，能够查看，修改，删除市区信息。

![](/md/blog.018.png)

图5.4 市区管理页面
### 5.1.5 修改密码
管理员进入指定功能操作区之后可以更改密码。其页面见下图。管理员更改密码之后，需要退出系统并重新登录。
...
## 5.2 员工功能实现
### 5.2.1 员工注册
员工可以先注册获取职工健康每日申报系统的账号，密码。其页面见下图。员工需要登记的信息包括姓名，邮箱，密码，工号等信息。

![](/md/blog.020.png)

图5.6 员工注册页面
### 5.2.2 新增健康信息
员工进入指定功能操作区之后可以新增健康信息。其页面见下图。员工登记自己的体温，身体状况，所在省份以及市区，详细住址等信息，最后提交健康登记信息。

![](/md/blog.021.png)

图5.7 新增健康信息页面
### 5.2.3 健康登记管理
员工进入指定功能操作区之后可以管理健康登记信息。其页面见下图。员工查看自己的身体状况信息，可以在本页面修改健康登记信息，查询健康登记信息。
